...
var MooTools={"version":"1.2dev","build":"1004"};var Native=function(I){var A=I.name||false;var D=I.initialize||false;var G=I.generics||true;var E=I.browser||false;
...
var C=E.properties||{};var B=$(E.iframe)||false;var D=C.onload||$empty;delete C.onload;C.id=C.name=C.id||C.name||B.id||B.name||"IFrame_"+IFrame.uid;((B=B||Element.create("iframe"))).set(C);
var A=function(){B.window=B.contentWindow;if(window.location.host==B.src.split("/")[2]){new Window(B.window);new Document(B.window.document);}D.call(B.window);
};if(!window.frames[C.id]){B.addListener("load",A);}else{A();}return B;}});IFrame.uid=0;var Elements=new Native({initialize:function(F,B){F=F||[];var E=F.length;
if(B||!E){return $extend(F,this);}var G={};var D=[];for(var A=0;A<E;A++){var C=$(F[A]);if(!C||G[C.$attributes.uid]){continue;}G[C.$attributes.uid]=true;
D.push(C);}return $extend(D,this);}});Elements.$multiply=function(A){return function(){var B=[];var F=true;for(var D=0,C=this.length;D<C;D++){var E=this[D][A].apply(this[D],arguments);
B.push(E);if(F){F=($type(E)=="element");}}return(F)?new Elements(B):B;};};Window.implement({$:function(B){if(B&&B.$attributes){return B;}var A=$type(B);
if(A=="string"){A=(B=this.document.getElementById(B))?"element":false;}if(A!="element"){return(A=="window"||A=="document")?B:null;}if(Garbage.collect(B)&&!B.$family){$extend(B,Element.prototype);
}return B;},$$:function(){var D=[];for(var C=0,B=arguments.length;C<B;C++){var A=arguments[C];switch($type(A)){case"element":D.push(A);break;case false:case null:break;
case"string":A=this.document.getElements(A,true);default:D.extend(A);}}return new Elements(D);}});Native.implement([Element,Document],{getElement:function(A,C){var B=this.getElementsByTagName(A)[0]||null;
...
},getPosition:function(){return{"x":0,"y":0};}});Native.implement([Element,Document],{getElements:function(L,K){var A=Selectors.Method;var B=[];L=L.split(",");
for(var I=0,H=L.length;I<H;I++){var D=L[I],J=[],G=[];D=D.trim().replace(Selectors.sRegExp,function(N){if(N.charAt(2)){N=N.trim();}G.push(N.charAt(0));return"%"+N.charAt(1);
}).split("%");for(var F=0,C=D.length;F<C;F++){var E=Selectors.parse(D[F]);if(!E){break;}var M=A.getParam(J,G[F-1]||false,this,E.tag,E.id,E.classes,E.attributes,E.pseudos);
if(!M){break;}J=M;}B=B.concat(A.getItems(J,this));}return(K)?B:new Elements(B);},getElement:function(A){return $(this.getElements(A,true)[0]||null);}});
...
if(!D){B.attributes.push([arguments[1],arguments[3]?"=":"",arguments[3]]);break;}var C={"name":arguments[1],"parser":D,"argument":arguments[3]};if(D.parser){C.argument=(D.parser.apply)?D.parser(C.argument):D.parser;
}B.pseudos.push(C);break;default:B.tag=E;}return"";});return B;};Selectors.Pseudo=new Hash;Selectors.XPath={getParam:function(G,F,C,L,B,A,D,H){var K=C.namespaceURI?"xhtml:":"";
...
}};Selectors.Filter={getParam:function(H,G,C,M,B,A,E,I){if(G){switch(G){case" ":H=Selectors.Filter.getNestedByTag(H,M);break;case">":H=Selectors.Filter.getChildrenByTag(H,M);
break;case"+":H=Selectors.Filter.getFollowingByTag(H,M);break;case"~":H=Selectors.Filter.getFollowingByTag(H,M,true);}if(B){H=Elements.filterById(H,B,true);
}}else{if(B){var D=C.getElementById(B,true);if(!D||((M!="*")&&(D.tagName.toLowerCase()!=M))){return false;}H=[D];}else{H=$A(C.getElementsByTagName(M));
}}var F;for(F=A.length;F--;F){H=Elements.filterByClass(H,A[F],true);}for(F=E.length;F--;F){var L=E[F];H=Elements.filterByAttribute(H,L[0],L[1],L[2],true);
}for(F=I.length;F--;F){var J=I[F];var K={};H=H.filter(function(O,N,P){return J.parser.filter(O,J.argument,N,P,K);});K=null;}return H;},getItems:function(A,B){return A;
},hasTag:function(B,A){return(B.nodeName&&B.nodeType==1&&(A=="*"||B.tagName.toLowerCase()==A));},getFollowingByTag:function(D,A,F){var G=[];for(var C=0,B=D.length,E;
C<B;C++){E=D[C].nextSibling;while(E){if(Selectors.Filter.hasTag(E,A)){G.push(E);if(!F){break;}}E=E.nextSibling;}}return G;},getChildrenByTag:function(G,A){var H=[];
for(var F=0,D=G.length;F<D;F++){var E=G[F].childNodes;for(var C=0,B=E.length;C<B;C++){if(Selectors.Filter.hasTag(E[C],A)){H.push(E[C]);}}}return H;},getNestedByTag:function(D,A){var E=[];
for(var C=0,B=D.length;C<B;C++){E.extend(D[C].getElementsByTagName(A));}return E;}};Selectors.Method=(Client.Features.xpath)?Selectors.XPath:Selectors.Filter;
